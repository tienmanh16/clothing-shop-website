...
public class ChangeServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
       ...
       String oldpass = request.getParameter("oldpass");
        ...
        DAO d = new DAO();
        User user = d.check(u, oldpass);
        if(user == null){
             String ms = "Mật khẩu hiện tại chưa chính xác !!!" + u + oldpass;
            request.setAttribute("ms", ms);
            request.getRequestDispatcher("account_edit.jsp").forward(request, response);
        }else{
             User userNew = new User(user.getRoleID(),user.getName(),user.getAddress(),user.getEmail(),user.getPhone(),u,newpass,2);
            d.change(userNew);
